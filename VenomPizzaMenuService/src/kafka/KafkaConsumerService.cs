...
public class KafkaConsumerService : BackgroundService
{
    ...
    {
        KafkaEvent<ProductDto>? dto = JsonSerializer.Deserialize<KafkaEvent<ProductDto>>(message, _jsonSerializerOptions);
        if (dto == null)
            throw new ArgumentException("Пустой json");
        _logger.LogInformation($"Обработка ивента типа {dto.EventType}");
        if (dto is { EventType: "product_added", Data: not null })
            await productsService.AddProduct(dto.Data);
        else if (dto is { EventType: "product_updated", Data: not null })
