...
internal class UpdateTests
{
    ...
    [Test]
    public async Task UpdateProductQuantityInCart_Success()
    {
        ...
        _mockProductsRepository.Setup(repository => repository.GetProductById(productId))
            .ReturnsAsync(new Product(productId, "Product") { PriceVariants=priceVariants });
        ...
        await _cartsService.UpdateProductQuantityInCart(cartId,productId,priceId,quantity);
    ...
    [Test]
    public void UpdateProductQuantityInCart_ProductNotFound()
    {
        int cartId = 1, productId = 2, priceId = 3, quantity = 4;
        _mockProductsRepository.Setup(repository => repository.GetProductById(productId))
            .ReturnsAsync(()=>null);
    ...
    [Test]
    public void UpdateProductQuantityInCart_PriceNotFound()
    {
        int cartId = 1, productId = 2, priceId = 3, quantity = 4;
